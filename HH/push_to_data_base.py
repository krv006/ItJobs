...
def insert_data_to_sql():
    csv_file = r"cleaned_job_titles_final.csv"
    conn = None
    cursor = None
    ...
    try:
        ...
        if not os.path.exists(csv_file):
            print(
                f"❌ ERROR: Final cleaned CSV file not found at '{csv_file}'. Did Matched_data.py run successfully and create output?")
            return

        job_data = pd.read_csv(csv_file, keep_default_na=False, encoding='utf-8')
        print(f"Loaded {len(job_data)} rows.")
        print(f"Columns found in final CSV: {job_data.columns.tolist()}")
        ...
        for idx, job in job_data.iterrows():
            ...
            try:
                ...
                print(
                    f"❌ ERROR: Failed preparing row {idx} (ID: {job_id_val}). Type: {type(row_error).__name__}, Msg: {row_error}")
